...
public class BillingController {
      ...
      for (PricingMenu menu : plan.getPricingMenus()) {
        for (PricingUnit unitObj : menu.getUnits()) {
          ...
          if (instance instanceof PricingFixedUnit) {
            ...
            type = unit.getType() != null ? unit.getType().getValue() : "fixed";
            ...
            type = unit.getType() != null ? unit.getType().getValue() : "usage";
            AggregateUsage usage = unit.getAggregateUsage();
            agg = usage != null ? usage.getValue() : "sum";
            currency = unit.getCurrency() != null ? unit.getCurrency().getValue() : "JPY";
            dispName = unit.getDisplayName();
          } else if (instance instanceof PricingTieredUnit) {
            PricingTieredUnit unit = (PricingTieredUnit) instance;
            unitName = unit.getMeteringUnitName();
            type = unit.getType() != null ? unit.getType().getValue() : "tiered";
            currency = unit.getCurrency() != null ? unit.getCurrency().getValue() : "JPY";
            AggregateUsage usage = unit.getAggregateUsage();
            agg = usage != null ? usage.getValue() : "sum";
            dispName = unit.getDisplayName();
            ...
            unitName = unit.getMeteringUnitName();
            type = unit.getType() != null ? unit.getType().getValue() : "tiered_usage";
            AggregateUsage usage = unit.getAggregateUsage();
            agg = usage != null ? usage.getValue() : "sum";
            currency = unit.getCurrency() != null ? unit.getCurrency().getValue() : "JPY";
            dispName = unit.getDisplayName();
          ...
          double count = usageCache.getOrDefault(unitName, 0.0);

          // 使用量取得（fixed以外）
          if (!"fixed".equals(type) && count == 0.0) {
            MeteringUnitDatePeriodCounts counts = meteringApi
                .getMeteringUnitDateCountByTenantIdAndUnitNameAndDatePeriod(
                    tenant_id, unitName, start, end);
  ...
  @GetMapping(value = "/tenant/plan_periods", produces = "application/json")
  ...
  @PostMapping(value = "/metering/{tenantId}/{unit}/{ts}", produces = "application/json")
  public ResponseEntity<?> updateMeteringCount(HttpServletRequest request,
